...
from mkdocs_editor_notes.version import (
    __version__,
    get_version,
...
def test_get_version__uses_pyproject():
    ...
    with mock.patch(
        "mkdocs_editor_notes.version.get_version_from_metadata", side_effect=PackageNotFoundError
    ) as mocked_metadata:
        with mock.patch(
            "mkdocs_editor_notes.version.get_version_from_pyproject", return_value=expected_version
        ) as mocked_pyproject:
...
def test_get_version__uses_metadata():
    expected_version = "1.2.3"
    with mock.patch(
        "mkdocs_editor_notes.version.get_version_from_metadata", return_value=expected_version
    ) as mocked_metadata:
        with mock.patch(
            "mkdocs_editor_notes.version.get_version_from_pyproject", side_effect=FileNotFoundError
        ) as mocked_pyproject:
            computed_version = get_version()
            assert computed_version == expected_version
